...
public struct Mover: Sendable {
    ...
    public enum MoveError: Error, Equatable {
        ...
        /// Dedup found a byte-identical copy already on the NAS, so the source was
        /// removed without copying again. Not a failure — counted as a dedupe.
        case duplicateRemoved(destination: URL)
        /// Verify-after-copy found the destination didn't match the source. The
        /// copy was discarded and the source left untouched for a retry.
    ...
    @discardableResult
    public func move(_ source: URL, to destination: String, policy: ConflictPolicy = .rename,
                     deduplicate: Bool = false, verify: Bool = false) throws -> URL {
        let destDir = resolveDestinationDirectory(destination)
        return try move(source, intoDirectory: destDir, policy: policy,
                        deduplicate: deduplicate, verify: verify)
    }
    ...
    @discardableResult
    public func move(_ source: URL, intoDirectory destDir: URL, policy: ConflictPolicy = .rename,
                     deduplicate: Bool = false, verify: Bool = false) throws -> URL {
        do {
        ...
        // Dedup short-circuit: if a same-named file is already there and is
        // byte-identical, this is a re-download — drop the source rather than make
        // another copy. Runs before the conflict policy (overwriting/renaming an
        // identical file is pointless work).
        if deduplicate, FileManager.default.fileExists(atPath: target.path),
           Checksum.filesAreIdentical(source, target) {
            guard source.path.withCString({ unlink($0) }) == 0 else {
                throw MoveError.sourceNotRemoved(destination: target)
            }
            throw MoveError.duplicateRemoved(destination: target)
        }
        ...
        // 1b. Optionally verify the copy byte-for-byte before we commit to it —
        //     guards against silent SMB corruption. Mismatch → discard, keep source.
        if verify, !Checksum.filesAreIdentical(source, tmp) {
            unlinkQuietly(tmp)
            throw MoveError.verifyFailed(dest.lastPathComponent)
        }
